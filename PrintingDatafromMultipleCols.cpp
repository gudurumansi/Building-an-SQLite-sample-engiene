//STEP-6: Reading Data From Multiple Columns
...
// Extract and print multiple columns from a table
void extractMultipleColumns(std::ifstream& file, unsigned rootPage, unsigned pageSize, const std::vector<int>& columnIndices) {
    ...
    std::cout << "\nValues from columns: ";
    for (int col : columnIndices) std::cout << col << " ";
...
int main() {
    ...
    std::string input;
    std::cout << "Enter comma-separated column indices (e.g., 0,2,3): ";
    ...
    while (std::getline(ss, token, ',')) {
        selectedCols.push_back(std::stoi(token));
    }

    extractMultipleColumns(file, tableList[choice].second, pageSize, selectedCols);
